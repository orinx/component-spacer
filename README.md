...
[![Build Status](https://travis-ci.org/mozilla-appmaker/component-spacer.png)](https://travis-ci.org/mozilla-appmaker/component-spacer)

A web component for [Appmaker](https://github.com/mozilla-appmaker/appmaker).

Appmaker import:
```
<link rel="import" href="/component-spacer">
```

# Test Status

See the github page http://mozilla-appmaker.github.io/component-spacer/ for test status.
...
Clone this repo:
```
git clone git@github.com:mozilla-appmaker/component-spacer.git
```
...
Your directory structure should look like this (more component repos can be cloned as siblings of `component-spacer`):
```
mozilla-appmaker/
  ├── appmaker/
  ├── component-spacer/
...
Enter the `component-spacer` directory, and install npm dependencies
```
cd component-spacer
npm install
```
...
execute: {
  click: function (channel) {
    ...
    spacerDivElement.$.spacerDiv.dispatchEvent(e);
  }
},
check: {
...
```
  var iframeHandler, spacerDivElement;

  beforeEach(function (done) {
    iframeHandler = harnessUtils.createIframe('test/html/test.html', function (win, doc) {
      spacerDivElement = iframeHandler.document.querySelector('ceci-spacer');
      done();
    });
  });
  ...
  describe('Ceci Spacer', function () {
    test('Sanity check', function (done) {
      chai.assert(spacerDivElement.ceci, 'Ceci descriptor exists.');
      iframeHandler.runIframeTest('Sanity Check', done);
    });

    test('Broadcasts', function (done) {
      iframeHandler.testBroadcasts(spacerDivElement, done, {
...
<body>
  ...
  <ceci-spacer id="ceci-spacer" value="you must construct additional pylons"></ceci-spacer>
</body>
</html>
```
...
<script>
  iframeTestUtils(function () {
    test('example', function (done) {
      var spacerDivElement = document.querySelector('ceci-spacer');
      chai.assert(spacerDivElement.$.spacerDiv, 'Spacer recognized and exposed by Polymer.');
      chai.assert(spacerDivElement.ceci.broadcasts, 'Ceci broadcasts publicized.');
      chai.assert(spacerDivElement.ceci.listeners, 'Ceci listeners publicized.');
      done();
    });
  });
</script>
